...
export function renderChartSvg(
  ...
  switch (chartType) {
    case 'pie':   return renderPie(data, config);
    case 'line':  return renderLine(data, config, false);
    case 'area':  return renderLine(data, config, true);
    default:      return renderBar(data, config);
  }
}
...
function renderBar(data: ChartData, config: ChartConfig): string {
  ...
  const bars = data.map((d, i) => {
    ...
    return [
      ...
      val > 0 ? `  <text x="${cx.toFixed(1)}" y="${(y - 3).toFixed(1)}" text-anchor="middle" fill="#475569" font-size="8" font-family="system-ui,sans-serif">${esc(val)}</text>` : '',
    ].join('\n');
  });
...
function renderPie(data: ChartData, config: ChartConfig): string {
  ...
  const slices = data.map((d, i) => {
    ...
    return [
      ...
      sweep > 0.2 ? `  <text x="${lx.toFixed(1)}" y="${ly.toFixed(1)}" text-anchor="middle" dominant-baseline="middle" fill="#fff" font-size="9" font-family="system-ui,sans-serif">${pct}%</text>` : '',
    ].join('\n');
  });
...
function renderLine(data: ChartData, config: ChartConfig, area: boolean): string {
  ...
  const xLabels  = pts
    ...
    .map((p) => `  <text x="${p.x.toFixed(1)}" y="${(PAD.top + INNER_H + 13).toFixed(1)}" text-anchor="middle" fill="#64748b" font-size="9" font-family="system-ui,sans-serif">${esc(p.label)}</text>`)
    .join('\n');
