...
export function buildDataAgentGraph(deps: DataAgentGraphDeps) {
  const { llm, structuredLlm, neoOntologyService, discoveryService, sandboxService, datasourceId, emit, tracer } = deps;

  const graph = new StateGraph(DataAgentState)
    .addNode('planner', createPlannerNode(structuredLlm, emit, tracer))
    .addNode('navigator', createNavigatorNode(llm, neoOntologyService, datasourceId, emit, tracer))
    .addNode('sql_builder', createSqlBuilderNode(structuredLlm, emit, tracer))
    .addNode('executor', createExecutorNode(llm, structuredLlm, discoveryService, sandboxService, emit, tracer))
  ...
  // ── planner → ? ───────────────────────────────────────────────────────────
  graph.addConditionalEdges('planner', (state: DataAgentStateType) => {
    const plan = state.plan;
    if (!plan) return 'explainer';
    if (plan.shouldClarify && plan.clarificationQuestions.length > 0) return '__end__';
  ...
  // ── navigator → ? ─────────────────────────────────────────────────────────
  graph.addConditionalEdges('navigator', (state: DataAgentStateType) => {
  ...
  // ── verifier → ? ──────────────────────────────────────────────────────────
  graph.addConditionalEdges('verifier', (state: DataAgentStateType) => {
    const report = state.verificationReport;
    if (!report || report.passed || state.revisionCount >= MAX_REVISIONS) return 'explainer';
