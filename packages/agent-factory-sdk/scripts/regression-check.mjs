 ...
 *                   sql | chart | tools | safety | intents | ask | latency | mustache-token | mustache-impact
 ...
 *   --project-id    Eval project id for both runs (default: qwery-core)
 *
 * Examples:
 *   node scripts/regression-check.mjs --base main --suite chart --open
 *   node scripts/regression-check.mjs --base HEAD^ --suite mustache-token --open
...
const model =
  arg('--model') ?? process.env.EVAL_MODEL ?? 'ollama-cloud/minimax-m2.5';
const strictMode = flag('--strict');
const overlaySrc = flag('--overlay-src');
const reportPathArg = arg('--report');
const projectId = arg('--project-id') ?? process.env.EVAL_PROJECT_ID ?? 'qwery-core';

if (!baseRef) {
  console.error('❌  --base <ref> is required');
  console.error(
    '   Example: node scripts/regression-check.mjs --base main --suite chart',
  );
  process.exit(1);
}
...
const SUITES = {
  sql: {
    script: 'evals-regression/suites/single-turn/sql-quality-eval-suite.ts',
    dataset: 'sql-quality-evals',
  },
  chart: {
    script: 'evals-regression/suites/single-turn/chart-generation-eval-suite.ts',
    dataset: 'chart-generation-evals',
  },
  tools: {
    script: 'evals-regression/suites/single-turn/tool-sequence-eval-suite.ts',
    dataset: 'tool-sequence-evals',
  },
  safety: {
    script: 'evals-regression/suites/single-turn/destructive-safety-eval-suite.ts',
    dataset: 'destructive-safety-evals',
  },
  intents: {
    script: 'evals-regression/suites/single-turn/expanded-intent-eval-suite.ts',
    dataset: 'expanded-intent-evals',
  },
  ask: {
    script: 'evals-regression/suites/single-turn/ask-eval-suite.ts',
    dataset: 'ask-agent-core-evals',
  },
  latency: {
    script: 'evals-regression/suites/single-turn/latency-token-eval-suite.ts',
    dataset: 'latency-token-metrics',
  },
  'mustache-token': {
    script: 'evals-regression/suites/single-turn/mustache-token-optimization-eval-suite.ts',
    dataset: 'mustache-token-optimization',
  },
  'mustache-impact': {
    script: 'evals-regression/suites/single-turn/mustache-impact-execution-eval-suite.ts',
    dataset: 'mustache-impact-execution-evals',
  },
  'multi-turn': {
    script: 'evals-regression/suites/multi-turn/multi-turn-ask-eval-suite.ts',
    dataset: 'multi-turn-ask-evals',
  },
  'context-retention': {
    script: 'evals-regression/suites/multi-turn/context-retention-eval-suite.ts',
    dataset: 'context-retention-evals',
  },
  'conv-correction': {
    script: 'evals-regression/suites/multi-turn/conversation-correction-eval-suite.ts',
    dataset: 'conversation-correction-evals',
  },
  bird: {
    script: 'evals-regression/suites/benchmarks/bird-eval-suite.ts',
    dataset: 'bird-mini_dev_sqlite',
  },
  'bird-schools': {
    script: 'evals-regression/suites/benchmarks/bird-california-schools.ts',
    dataset: 'bird-mini_dev_sqlite',
  },
  'bird-formula1': {
    script: 'evals-regression/suites/benchmarks/bird-formula1.ts',
    dataset: 'bird-mini_dev_sqlite',
  },
};
...
console.log(
  `    Overlay:   ${
    overlaySrc ? 'on (compare prompts/tools only)' : 'off (true before/after code diff)'
  }`,
);
console.log('');
...
try {
  try {
    exec('git worktree prune', REPO_ROOT);
  } catch {
    // non-fatal
  }
  ...
  cpSync(
    join(PKG_ROOT, 'evals-regression'),
    join(BASELINE_PKG, 'evals-regression'),
    {
      recursive: true,
      filter: (src) =>
        !src.includes('node_modules') ||
        src.includes(join('node_modules', '@qwery', 'shared')),
    },
  );

  if (overlaySrc) {
    console.log('🔗  Copying src/ into baseline worktree (overlay mode)…');
    cpSync(join(PKG_ROOT, 'src'), join(BASELINE_PKG, 'src'), {
      recursive: true,
      filter: (src) => !src.includes('node_modules'),
    });
    console.log(
      '🔗  Copying packages/tracing-sdk into baseline worktree (overlay mode)…',
    );
    cpSync(
      join(REPO_ROOT, 'packages', 'tracing-sdk'),
      join(WORKTREE_PATH, 'packages', 'tracing-sdk'),
      {
        recursive: true,
        filter: (src) => !src.includes('node_modules'),
      },
  ...
  if (!existsSync(baselineRootNm)) {
    ...
    execSync(`mklink /J "${baselineRootNm}" "${headRootNm}"`, {
      shell: SHELL,
      stdio: 'inherit',
    });
  }
  if (!existsSync(baselinePkgNm)) {
    console.log(
      '🔗  Linking packages/agent-factory-sdk/node_modules into baseline worktree…',
    );
    execSync(`mklink /J "${baselinePkgNm}" "${headPkgNm}"`, {
      shell: SHELL,
      stdio: 'inherit',
    });
  }
  ...
  for (const s of suitesToRun) {
    ...
    console.log(`   › ${suiteDef.script}`);
    const result = spawnSync('bun', [join(BASELINE_PKG, suiteDef.script)], {
      cwd: PKG_ROOT,
      stdio: 'inherit',
      env: buildSuiteEnv(model, versionBefore),
      shell: true,
    });
    if (result.status !== 0) {
      console.warn(
        `   ⚠  ${suiteDef.script} exited with code ${result.status} (continuing)`,
      );
      failures.push({
        phase: 'before',
        suite: s,
        script: suiteDef.script,
...
for (const s of suitesToRun) {
  ...
  if (!suiteDef) continue;
  console.log(`   › ${suiteDef.script}`);
  const result = spawnSync('bun', [suiteDef.script], {
    cwd: PKG_ROOT,
    stdio: 'inherit',
    env: buildSuiteEnv(model, versionAfter),
    shell: true,
  });
  if (result.status !== 0) {
    console.warn(
...
if (suitesToRun.includes('mustache-token') || suitesToRun.includes('mustache-impact')) {
  console.log('');
  console.log('🧮  Compiling Mustache impact report…');
  const result = spawnSync(
    'node',
    [
      'evals-regression/reports/compile-mustache-impact-report.mjs',
      '--before',
      versionBefore,
      '--after',
      versionAfter,
  ...
  if (result.status !== 0) {
    console.warn(
      `   ⚠  Mustache impact report compilation exited with code ${result.status}`,
    );
    failures.push({
      phase: 'report',
      suite: 'mustache-token',
      script: 'evals-regression/reports/compile-mustache-impact-report.mjs',
      exitCode: result.status,
    });
  }
}
...
for (const s of suitesToRun) {
  const suiteDef = SUITES[s];
  if (!suiteDef) continue;
  const url = `${UI_BASE_URL}/compare?dataset=${encodeURIComponent(
    suiteDef.dataset,
  )}&vA=${encodeURIComponent(versionBefore)}&vB=${encodeURIComponent(versionAfter)}`;
  comparisonUrls.push({ suite: s, dataset: suiteDef.dataset, url });
  console.log(`   ${s.padEnd(18)} → ${url}`);
...
if (failures.length > 0) {
  ...
  for (const item of failures) {
    console.log(
      `   - [${item.phase}] ${item.suite} (${item.script}) exit=${item.exitCode}`,
    );
  }
  if (strictMode) {
    process.exit(2);
...
function buildSuiteEnv(modelString, agentVersion) {
  ...
  if (provider === 'azure') {
    env.AZURE_OPENAI_DEPLOYMENT =
      env.AZURE_OPENAI_DEPLOYMENT ?? modelName;
  }
...
function removeWorktree() {
  const rmCmd =
    process.platform === 'win32'
      ...
      : `rm -rf "${WORKTREE_PATH}"`;
  try {
    execSync(rmCmd, { cwd: REPO_ROOT, shell: SHELL, stdio: 'inherit' });
  } catch {
    // ignore
  }
  try {
...
function patchBaselinePackageJson(baselinePkgDir) {
  ...
  const baselinePkg = JSON.parse(
    readFileSync(join(baselinePkgDir, 'package.json'), 'utf8'),
  );
  const evalScripts = Object.fromEntries(
    Object.entries(headPkg.scripts ?? {}).filter(([key]) =>
      key.startsWith('eval:suite'),
    ),
  );
  baselinePkg.scripts = { ...(baselinePkg.scripts ?? {}), ...evalScripts };
  writeFileSync(
    join(baselinePkgDir, 'package.json'),
