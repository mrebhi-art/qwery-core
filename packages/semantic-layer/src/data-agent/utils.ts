...
export function fieldsToYaml(name: string, source: string, fieldsJson: string): string {
  ...
  for (const f of fields) {
    ...
    if (field['description']) lines.push(`    description: ${String(field['description'])}`);
    if (field['dataType']) lines.push(`    dataType: ${String(field['dataType'])}`);
...
export function rowsToTable(columns: string[], rows: Record<string, unknown>[], maxRows = 100): string {
...
export function getDialectFromDriverId(driverId: string): string {
  if (driverId.startsWith('postgresql') || driverId.startsWith('pglite')) return 'postgresql';
  if (driverId.startsWith('mysql')) return 'mysql';
  if (driverId.startsWith('clickhouse')) return 'clickhouse';
  if (driverId.startsWith('duckdb') || driverId.startsWith('s3') || driverId.startsWith('azure')) return 'duckdb';
