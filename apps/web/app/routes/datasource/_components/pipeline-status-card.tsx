...
type StageStatus = 'not_started' | 'pending' | 'running' | 'indexing' | 'ready' | 'failed';

function StatusBadge({ status }: { status: StageStatus }) {
  const baseClass = 'inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium';

  if (status === 'ready') {
    return <span className={`${baseClass} bg-green-500/15 text-green-600 dark:text-green-400`}>Ready</span>;
  }
  if (status === 'running' || status === 'indexing' || status === 'pending') {
    return (
      <span className={`${baseClass} bg-yellow-500/15 text-yellow-600 dark:text-yellow-400`}>
  ...
  if (status === 'failed') {
    return <span className={`${baseClass} bg-destructive/15 text-destructive`}>Failed</span>;
  }
  return <span className={`${baseClass} bg-muted text-muted-foreground`}>Not started</span>;
}
...
export function PipelineStatusCard({ datasourceId }: Props) {
  ...
  const stages = [
    {
      key: 'discovery',
      label: t('datasource.pipeline.discovery', { defaultValue: 'Schema Discovery' }),
    ...
    {
      key: 'semanticModel',
      label: t('datasource.pipeline.semanticModel', { defaultValue: 'Semantic Model' }),
      ...
      action: (
        <Button
          ...
          disabled={data.semanticModel.status === 'running' || rebuildSM.isPending}
          onClick={() => rebuildSM.mutate()}
          data-test="rebuild-semantic-model"
        >
          <RefreshCw className={`mr-1 h-3 w-3 ${rebuildSM.isPending ? 'animate-spin' : ''}`} />
    ...
    {
      key: 'ontology',
      label: t('datasource.pipeline.ontology', { defaultValue: 'Ontology Index' }),
      ...
      action: (
        <Button
          ...
          size="sm"
          className="h-7 px-2 text-xs"
          disabled={data.ontology.status === 'indexing' || data.ontology.status === 'running' || rebuildOntology.isPending}
          onClick={() => rebuildOntology.mutate()}
          data-test="rebuild-ontology"
        >
          <RefreshCw className={`mr-1 h-3 w-3 ${rebuildOntology.isPending ? 'animate-spin' : ''}`} />
          {t('datasource.pipeline.rebuild', { defaultValue: 'Rebuild' })}
        </Button>
      ),
    },
        ...
        <div className="space-y-2">
          {stages.map((stage) => (
            <div key={stage.key} className="flex items-center gap-3">
              ...
              {stage.time && (
                <span className="text-muted-foreground text-xs">{formatTime(stage.time)}</span>
              )}
              {stage.error && (
                <span className="text-destructive max-w-xs truncate text-xs" title={stage.error}>
                  {stage.error}
                </span>
              )}
